            ...
            if (tokens != null && !"".equals(tokens)) {
                ...
                if (status == 0) {
                    if (!StringUtils.isEmpty(json.getString("errcode"))) {
                        ...
                        resMap.put("errMsg", "获取媒体数据失败！");
                        return resMap;
                    }
                    JSONObject doc = json.getJSONObject("data");
                    resMap.put("result", doc);
                    logger.info("------------------------ query api: {}, page: {}, pageSize: {}, total: {}, pageCount: {}  ------------------",
                            url, doc.getString("page"), doc.getString("pageSize"), doc.getString("total"),
                            doc.getString("pageCount"), doc.getString("total"));
                } else {
                    logger.info("------------------------ query api: {}, request failure: {} ------------------", url);
                }
            }
        } catch (SocketTimeoutException e) {
            logger.error("========================== callForienApi url: {}, occure error, message: call api timeout =====================", url);
            resMap.put("errMsg", "超时");
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("---------------------- query api: {}, occure exception, error: {} --------------------", url, e.getMessage());
            resMap.put("errMsg", "失败");
        }
        return resMap;
    }
